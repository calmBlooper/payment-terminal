...
void PrivateAccountRepository::retrieveData() {
	data.clear();
...
PrivateAccount PrivateAccountRepository::getByKey(const std::pair<std::string, std::string>& nameSurename) {
	char* zErrMsg = 0;
	int rc = 0;
...
std::vector<PrivateAccount> PrivateAccountRepository::getAll() {
	char* zErrMsg = 0;
	int rc = 0;
	sqlite3* DB_connection = (DBConnector::GetInstance()->getConnection());
...
void PrivateAccountRepository::insert(const PrivateAccount& entity) {
	char* zErrMsg = 0;
	int rc = 0;
	sqlite3* DB_connection = (DBConnector::GetInstance()->getConnection());
...
void PrivateAccountRepository::remove(const PrivateAccount& entity) {
	char* zErrMsg = 0;
	int rc = 0;
	sqlite3* DB_connection = (DBConnector::GetInstance()->getConnection());
...
void PrivateAccountRepository::update(const PrivateAccount& entity) {
	char* zErrMsg = 0;
	int rc = 0;
	sqlite3* DB_connection = (DBConnector::GetInstance()->getConnection());
...
PrivateAccount PrivateAccountRepository::getById(const long long& id) {
	char* zErrMsg = 0;
	int rc = 0;
	sqlite3* DB_connection = (DBConnector::GetInstance()->getConnection());
